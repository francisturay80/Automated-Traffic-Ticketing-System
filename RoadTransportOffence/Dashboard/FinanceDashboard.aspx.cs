...
namespace RoadTransportOffence.Dashboard
{
    public partial class FinanceDashboard : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Convert.ToString(Session["Role"]) != "Accountant")
            {
                Session.Clear();
                Session.Abandon();
                Session.RemoveAll();
            ...
            //    using (conn)
            //    {
            //        SqlDataAdapter ad = new SqlDataAdapter("SELECT * FROM vwTopOffences", conn);
            //        ad.Fill(dt);
            //    }
            //    rptTopOffences.DataSource = dt;
            //    rptTopOffences.DataBind();
            //}
        ...
        public static List<SalesDetails> GetChartData()
        {
            ...
            using (SqlConnection con = new SqlConnection(ConnectionString))
            {
                ...
                //cmd.Parameters.AddWithValue("@GroupName", prefixText);
                da.Fill(dt);
                con.Close();
            }
